...
static s_light_ctx	build_light_ctx(const RenderContext *ctx, Vector3 *light_dir, Vector3 *cam_dir)
{
	s_light_ctx	l_ctx;
	...
	l_ctx.attenuation = ctx->funcs.calculate_attenuation(l_ctx.distance_light, L_P_KC, L_P_KL, 0);
	l_ctx.diffuse_intensity = ctx->funcs.calculate_intensity(ctx->normal, *light_dir);
	l_ctx.diffuse_intensity = fmax(fmin(l_ctx.diffuse_intensity, 1.0), 0);
	l_ctx.reflect_dir = ctx->funcs.reflect(*light_dir, ctx->normal);
	l_ctx.specular = specular_intensity(*l_ctx.reflect_dir, *cam_dir, SHININESS, KS);
...
Color* apply_lighting(const RenderContext *ctx, Vector3 *light_dir, Vector3 *cam_dir)
{
	...
	current_color = illuminate_surface(current_color, l_ctx.light->color, \
		fmax(fmin(1 - (l_ctx.light->ratio * l_ctx.diffuse_intensity), 1.0), 0), 1, 0, ctx->mater_prop);
	normalize_color(current_color);
	current_color = illuminate_surface(current_color, l_ctx.light->color, \
		fmax(fmin(1 - (l_ctx.light->ratio * l_ctx.specular), 1.0), 0), 1, 0, ctx->mater_prop);
	normalize_color(current_color);
