...
{
	double d;
	int current_color = scene.triangle[n_triangle].mater_prop.color[0];
	Color *vCurrentColor = scene.triangle[n_triangle].mater_prop.vColor;
	MaterialProperties prop = scene.triangle[n_triangle].mater_prop;
	int ambient_color = 0;
	Vector3 *cam_dir = normalize_withpoint(scene.cameras->pos, hit_pt);
	double intensity = calculate_intensity(scene.triangle[n_triangle].p_triangle->normal, *cam_dir);
	...
	if (intersect_triangle(&rayslight, &scene.triangle[n_triangle], &d))
	{
		...
		if (!t)
		{
				...
				double diffuse_intensity = calculate_intensity(scene.triangle[n_triangle].p_triangle->normal, rayslight.direction);
				diffuse_intensity = fmin(fmax(diffuse_intensity, 0.0), 1.0);
				Vector3 *reflect_dir = reflect(rayslight.direction, scene.triangle[n_triangle].p_triangle->normal);
